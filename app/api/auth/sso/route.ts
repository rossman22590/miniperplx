...
export async function GET(request: NextRequest) {
  ...
  // BETTER_AUTH_SECRET doubles as both the SSO signing key and the cookie signing key
  // (matches DATAVIBES_SECRET_KEY set on the AiTutor wrapper side)
  const secret = process.env.BETTER_AUTH_SECRET;
  if (!secret) {
    return NextResponse.json({ error: 'BETTER_AUTH_SECRET is not configured' }, { status: 500 });
  }

  // Verify HMAC-SHA256 signature (generated by AiTutor wrapper using DATAVIBES_SECRET_KEY)
  ...
  try {
    // 3. Look up the user in Datavibes's DB
    ...
    //    better-auth stores the RAW token in DB; the SIGNED version goes in the cookie
    ...
    // 5. Sign the session token exactly as better-auth does internally.
    //    getSignedCookie splits on last '.', verifies base64 HMAC, returns raw token to look up in DB.
    const signedTokenValue = await signCookieValue(sessionToken, secret);
